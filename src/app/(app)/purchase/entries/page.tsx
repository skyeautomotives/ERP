...
export default async function PurchaseEntriesPage({
  ...
  let query = supabase
    ...
    .select(
      "id, our_reference_number, supplier_invoice_number, supplier_invoice_date, total_amount, status, suppliers(name)",
      { count: "exact" },
    );

  if (q) query = query.or(`our_reference_number.ilike.%${q}%,supplier_invoice_number.ilike.%${q}%`);
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Purchase Entries</h1>
        {can(user, "purchase", "create") && (
          <Link
            href="/purchase/entries/new"
      ...
      <div className="mt-4">
        <SearchInput placeholder="Search reference or supplier invoice number..." />
      </div>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {(invoices ?? []).map((inv) => (
              <tr key={inv.id} className="hover:bg-gray-50 dark:hover:bg-gray-900">
                ...
                <td className="px-4 py-2 text-gray-500 dark:text-gray-400">{inv.supplier_invoice_number}</td>
                <td className="px-4 py-2 text-gray-500 dark:text-gray-400">{inv.supplier_invoice_date}</td>
