...
export default async function DebitNoteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: note }, { data: items }] = await Promise.all([
    supabase
      .from("debit_notes")
      .select("*, suppliers(name), purchase_invoices(our_reference_number)")
      .eq("id", id)
      .single(),
    supabase.from("debit_note_items").select("*, products(code, name)").eq("debit_note_id", id).order("id"),
  ]);
    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{note.debit_note_number}</h1>
            <HelpButton content={HELP_CONTENT["debit-notes"]} />
          </div>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Against {note.purchase_invoices?.our_reference_number} - {note.debit_note_date}
          </p>
        </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-6 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-6 text-sm">
        <div>
          ...
          <p className="font-medium text-gray-900 dark:text-gray-100">{note.suppliers?.name}</p>
        </div>
        <div>
