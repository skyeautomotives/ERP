...
import { createDebitNote } from "./actions";
...
export function DebitNoteForm({ purchaseInvoiceId, items }: { purchaseInvoiceId: string; items: LineItem[] }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const returnItems = Object.entries(quantities)
      .filter(([, v]) => Number(v) > 0)
      .map(([id, v]) => ({ purchase_invoice_item_id: id, quantity: Number(v) }));

    if (returnItems.length === 0) return setError("Enter a quantity to return for at least one item.");

    startTransition(async () => {
      const result = await createDebitNote({ purchase_invoice_id: purchaseInvoiceId, reason, items: returnItems });
