...
export default async function BankPaymentsPage({
  ...
  const { data: payments, count } = await supabase
    .from("payments")
    .select(
      "id, payment_number, payment_date, amount, purpose, status, paid_to, reference_number, suppliers(name), expense_categories(name)",
      { count: "exact" },
    )
    .eq("method", "bank")
    ...
    <div>
      ...
      <div className="mt-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h1 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Bank Payment</h1>
          <HelpButton content={HELP_CONTENT["payments"]} />
        </div>
        {can(user, "accounts", "create") && (
          <Link
            href="/accounts/payments/bank/new"
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {(payments ?? []).map((p) => (
              <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-900">
                ...
                <td className="px-4 py-2 text-gray-500 dark:text-gray-400">{p.reference_number ?? "-"}</td>
