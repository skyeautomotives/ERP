...
export default async function BankReceiptsPage({
  ...
  const { data: receipts, count } = await supabase
    .from("receipts")
    .select("id, receipt_number, receipt_date, amount, mode, status, reference_number, customers(name)", {
      count: "exact",
    })
    .eq("method", "bank")
    ...
    <div>
      ...
      <div className="mt-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h1 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Bank Receipt</h1>
          <HelpButton content={HELP_CONTENT["receipts"]} />
        </div>
        {can(user, "accounts", "create") && (
          <Link
            href="/accounts/receipts/bank/new"
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {(receipts ?? []).map((r) => (
              <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-900">
                ...
                <td className="px-4 py-2 text-gray-500 dark:text-gray-400">{r.reference_number ?? "-"}</td>
