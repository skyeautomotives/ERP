-- Phase 6: create_payment / cancel_payment. Purpose decides which fields matter:
-- 'supplier' needs supplier_id + invoice allocations, 'on_account' needs a
-- supplier_id but no allocations, 'expense' needs an expense_category_id.
...
begin
  ...
  insert into public.payments (
    payment_number, method, purpose, supplier_id, expense_category_id, paid_to,
    amount, payment_date, reference_number, notes, created_by
  ) values (
    v_payment_number, p_method, p_purpose, p_supplier_id, p_expense_category_id, p_paid_to,
...
revoke all on function public.create_payment(text, text, uuid, uuid, text, numeric, date, text, text, jsonb) from public;
grant execute on function public.create_payment(text, text, uuid, uuid, text, numeric, date, text, text, jsonb) to authenticated;
