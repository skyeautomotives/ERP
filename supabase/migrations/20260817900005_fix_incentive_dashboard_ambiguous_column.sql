-- Fix: get_incentive_dashboard failed with "column reference staff_id is
-- ambiguous" (42702) - the function's own RETURNS TABLE column staff_id
-- becomes an implicit PL/pgSQL identifier, colliding with unqualified
-- staff_id references inside the CTEs' final UNION and the targets CTE.
-- Re-applying the corrected function body (all staff_id references now
-- qualified with their table/alias).
...
begin
  ...
  targets as (
    select sst.staff_id, sum(sst.target_amount) as target_amount
    from public.staff_sales_targets sst
    where make_date(sst.period_year, sst.period_month, 1) between date_trunc('month', p_from)::date and date_trunc('month', p_to)::date
    group by sst.staff_id
  )
  select
    up.id,
    up.full_name,
  ...
  where up.id in (
    select staff_invoices.staff_id from staff_invoices
    union select collections.staff_id from collections
    union select outstanding_90.staff_id from outstanding_90
    union select targets.staff_id from targets
  )
