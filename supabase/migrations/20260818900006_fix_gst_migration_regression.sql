-- 20260818900004_editable_line_gst.sql accidentally reverted the WHERE-clause
-- fix from 20260817200006_fix_invoice_number_update.sql (copied the pre-fix
-- version of create_sales_invoice as its starting point). That file has been
-- corrected in place for future/fresh installs, but this database already
-- applied the broken version - re-declaring the fixed function here since
-- supabase db push does not re-run an already-applied migration file.
