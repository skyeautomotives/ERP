-- Fix: Supabase's managed Postgres rejects UPDATE statements with no WHERE clause
-- ("UPDATE requires a WHERE clause"), even from within a security-definer function.
-- company_settings is a true singleton (enforced by the unique index on (true) from
-- Phase 1), so filtering to "the one row that exists" is safe and always correct.
